...
import { Building2, Zap, Plus, Star } from "lucide-react";
...
export function PipelineSelector({ 
  ...
  // Mutation to make pipeline default
  const makeDefaultMutation = useMutation({
    mutationFn: async (pipelineId: string) => {
      ...
      // Then set the selected pipeline as default
    ...
    onError: (error) => {
      toast({
        title: "Error", 
        description: "Failed to set pipeline as default",
        variant: "destructive",
      });
    },
  });

  const handleMakeDefault = (e: React.MouseEvent, pipelineId: string) => {
    e.stopPropagation();
    makeDefaultMutation.mutate(pipelineId);
  };
